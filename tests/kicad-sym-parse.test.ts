...
import { parseKicadSymToSchematicInfo } from "src"
import fs from "fs"
...
test("parse resistor kicad_sym - 2 pin passive", () => {
  ...
  expect(info.schPinArrangement.topSide?.pins).toContain(1)
  expect(info.schPinArrangement.bottomSide?.pins).toContain(2)
})
...
test("parse ATmega328P kicad_sym - multi-pin IC", () => {
  ...
  expect(info.schPinArrangement.leftSide).toBeDefined()
  expect(info.schPinArrangement.leftSide!.pins).toContain(7) // VCC
  expect(info.schPinArrangement.leftSide!.pins).toContain(8) // GND
  expect(info.schPinArrangement.leftSide!.pins).toContain(1) // RESET

  // Right side: port pins (rotation 180°)
  expect(info.schPinArrangement.rightSide).toBeDefined()
  expect(info.schPinArrangement.rightSide!.pins).toContain(14) // PB0
  expect(info.schPinArrangement.rightSide!.pins).toContain(2) // PD0
})
