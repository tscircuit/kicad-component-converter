...
import { parseKicadComponentToCircuitJson } from "src/parse-kicad-mod-to-circuit-json"
...
export const App = () => {
  ...
  const handleProcessAndViewFiles = useCallback(async () => {
    if (!filesAdded.kicad_mod && !filesAdded.kicad_sym) {
      setError("No KiCad footprint or symbol file added")
      return
    }
    setError(null)
    let circuitJson: any
    try {
      circuitJson = await parseKicadComponentToCircuitJson({
        kicadMod: filesAdded.kicad_mod,
